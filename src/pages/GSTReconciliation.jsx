...
export default function GSTReconciliation() {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ FIXED: localhost ki jagah VITE_API_URL use karo
    ...
    try {
      const res = await fetch(`${API_URL}/api/query`, {
        ...
        body: JSON.stringify({
          ...
          message: formData.message || "GST Reconciliation Query",
          ...
          <main className="lg:w-2/3 w-full space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg border-t-4 border-green-600 p-6 sm:p-8">
              <h1 className="text-3xl sm:text-4xl font-bold text-slate-800 mb-4">
                {t("gstReconciliation.title")}
              </h1>
              <p className="text-slate-600 text-base leading-relaxed mb-4">
                {t("gstReconciliation.intro")}
              </p>
              <Link
                to="/query-form"
                className="inline-block bg-green-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-green-700 transition"
              >
                {t("gstReconciliation.consult_btn")}
              </Link>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
              <h2 className="text-2xl font-semibold text-slate-800 mb-4">
                {t("gstReconciliation.what_title")}
              </h2>
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
                {t("gstReconciliation.what_desc")}
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
              <h2 className="text-2xl font-semibold text-slate-800 mb-4">
                {t("gstReconciliation.why_title")}
              </h2>
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
                {t("gstReconciliation.why_desc")}
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
              <h2 className="text-2xl font-semibold text-slate-800 mb-6">
                {t("gstReconciliation.process_title")}
              </h2>
              <div className="grid sm:grid-cols-2 gap-6">
                {steps.map((key, index) => (
                  <div key={key} className="bg-green-50 p-5 rounded-xl">
                    <h3 className="font-semibold text-green-700 mb-2 text-lg">
                      {t("gstReconciliation.step")} {index + 1}
                    </h3>
                    <p className="text-sm text-slate-600">
                      {t(`gstReconciliation.${key}`)}
                    </p>
            ...
            <div className="bg-green-50 border border-green-200 rounded-2xl shadow-md p-6 text-center">
              <h2 className="text-xl sm:text-2xl font-semibold text-green-700 mb-3">
                {t("gstReconciliation.cta_title")}
              </h2>
              <p className="text-gray-700 mb-5 text-sm sm:text-base">
                {t("gstReconciliation.cta_desc")}
              </p>
              <Link
                to="/query-form"
                className="inline-block bg-green-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-green-700 transition"
              >
                {t("gstReconciliation.cta_btn")}
              </Link>
            </div>
          ...
          <aside className="lg:w-1/3 w-full">
            <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 sticky top-6">
              ...
              <h3 className="text-xl font-semibold text-slate-800 mb-4">
                {t("gstReconciliation.sidebar_title")}
              </h3>

              <p className="text-sm text-slate-600 mb-6">
                {t("gstReconciliation.sidebar_desc")}
              </p>
              ...
              {success ? (
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                  ...
                  <input
                    ...
                    type="text"
                    placeholder={t("gstReconciliation.name")}
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    type="tel"
                    placeholder={t("gstReconciliation.mobile")}
                    className="w-full border rounded-lg px-4 py-2"
                    required
                  />
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    type="email"
                    placeholder={t("gstReconciliation.email")}
                    className="w-full border rounded-lg px-4 py-2"
                    required
                  />
                  ...
                  >
                    {loading ? "Sending..." : t("gstReconciliation.sidebar_btn")}
                  </button>
